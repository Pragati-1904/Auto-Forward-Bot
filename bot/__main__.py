...
LOGS.info("Bot started. Userbot active: %s", _bot_pkg.userbot is not None)

try:
    # bot.run_until_disconnected() keeps the event loop alive for BOTH clients
    # (bot + userbot share the same loop, so userbot stays connected too)
